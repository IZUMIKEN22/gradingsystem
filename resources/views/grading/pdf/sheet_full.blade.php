                ...
                <tbody>
                    @for ($i = 0; $i < 25; $i++)
                        @php
                            ...
                            $right = $students[$i + 25] ?? null;
                        @endphp
                        <tr>
                            <td class="center">{{ $i + 1 }}</td>
                            <td>{{ $left->student_name ?? '' }}</td>
                            <td class="center">{{ $left ? number_format(gradeEquivalent($left->midterm_grade), 2) : '' }}
                            </td>
                            <td class="center">{{ $left ? number_format(gradeEquivalent($left->final_grade), 2) : '' }}</td>
                            <td class="center">{{ $left ? ($left->final_grade >= 75 ? 'PASSED' : 'FAILED') : '' }}</td>

                            <td class="center">{{ $i + 26 }}</td>
                            <td>{{ $right->student_name ?? '' }}</td>
                            <td class="center">{{ $right ? number_format(gradeEquivalent($right->midterm_grade), 2) : '' }}
                            </td>
                            <td class="center">{{ $right ? number_format(gradeEquivalent($right->final_grade), 2) : '' }}
                            </td>
                            <td class="center">{{ $right ? ($right->final_grade >= 75 ? 'PASSED' : 'FAILED') : '' }}</td>
                        </tr>
                    @endfor
                </tbody>
            </table>
