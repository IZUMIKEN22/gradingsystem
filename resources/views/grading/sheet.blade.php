...
@php
    ...
    // Statistics
    $passed = $realStudents->where('final_grade', '>=', 75)->count();
    $failed = $realStudents->where('final_grade', '<', 75)->count();
@endphp

<div class="bg-white p-6 text-sm mx-auto" style="max-width: 700px">
<a href="{{ route('grades.download.full', $class_id) }}" class="bg-purple-500 text-white px-4 py-2 rounded">Download Full PDF</a>
    ...
    <table class="w-full border border-black text-xs mb-3">
        <thead>
            <tr class="font-bold text-center">
                ...
                <td colspan="3" class="border border-black">Grades</td>
                <td rowspan="2" class="border border-black w-6">No.</td>
                <td rowspan="2" class="border border-black">Student Name</td>
                <td colspan="3" class="border border-black">Grades</td>
            </tr>
            <tr class="font-bold text-center">
                ...
                <td class="border border-black w-16">Finals</td>
            ...
            @for ($i = 0; $i < $totalRows; $i++)
                ...
                <tr>
                    ...
                    <td class="border border-black text-center font-bold">
                        @if($left && $left->midterm_grade !== null)
                            {{ number_format(gradeEquivalent($left->midterm_grade), 2) }}
                        @endif
                    </td>
                    <td class="border border-black text-center font-bold">
                        @if($left && $left->final_grade !== null)
                            {{ number_format(gradeEquivalent($left->final_grade), 2) }}
                        @endif
                    </td>
                    <td class="border border-black text-center font-bold
                        @if($left && $left->final_grade !== null)
                            {{ $left->final_grade >= 75 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}
                        @endif
                    ">
                        @if($left && $left->final_grade !== null)
                            {{ $left->final_grade >= 75 ? 'PASSED' : 'FAILED' }}
                        @endif
                    </td>
                    ...
                    <td class="border border-black text-center font-bold">
                        @if($right && $right->midterm_grade !== null)
                            {{ number_format(gradeEquivalent($right->midterm_grade), 2) }}
                        @endif
                    </td>
                    <td class="border border-black text-center font-bold">
                        @if($right && $right->final_grade !== null)
                            {{ number_format(gradeEquivalent($right->final_grade), 2) }}
                        @endif
                    </td>
                    <td class="border border-black text-center font-bold
                        @if($right && $right->final_grade !== null)
                            {{ $right->final_grade >= 75 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}
                        @endif
                    ">
                        @if($right && $right->final_grade !== null)
                            {{ $right->final_grade >= 75 ? 'PASSED' : 'FAILED' }}
                        @endif
                    </td>
                </tr>
            @endfor
